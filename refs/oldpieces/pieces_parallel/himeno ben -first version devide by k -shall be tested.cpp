...
float   omega=0.8;
Matrix  a,b,c,p,p2,bnd,wrk1,wrk2;
...
void *doStencil(void *param) {
    ...
    for(n=0 ; n<nn ; n++){
            ...
            std::cout<< "this is worker :"<<id<<"updating "<<" taking from "<<startk<<" to "<<endk<<std::endl;
            Barrier();
            for(i=1 ; i<imax ; i++)
                for(j=1 ; j<jmax ; j++)
                    for(k=startk ; k<=endk ; k++)
...
int main(int argc, char** argv) {
    ...
    */
    ...
    for(int i = 0; i < numWorkers; i++) {
        ...
        arg[i].wrk1 = &wrk1;
        arg[i].wrk2 = &wrk2;
        arg[i].startk = ((kmax-1)/numWorkers)*i + 1;// the real number , that computes
        arg[i].endk = ((kmax-1)/numWorkers)*(i + 1);
		std::cout<< "starting thread "<<i<<std::endl;
        pthread_create(&workerid[i], NULL, doStencil, (void*)&arg[i]);
    }

        // main take the rest numbers 
        struct arguments argmin;
        argmin.nn = nn;
        argmin.id = -1;//set the id of main = -1 or some others (not used later)
        argmin.a = &a;
        argmin.b = &b;
        argmin.c = &c;
        argmin.p = &p2;
        ...
        argmin.wrk1 = &wrk1;
        argmin.wrk2 = &wrk2;
        argmin.startk = ((kmax-1)/numWorkers)*numWorkers + 1;// the real number , that computes
        argmin.endk = (kmax-1);
        if(argmin.startk<=argmin.endk){
            std::cout<< "main thread starts computing k from "<<argmin.startk<<" to "<<argmin.endk<<std::endl;
            doStencil((void*)&argmin);
        }
    ...
    */
    ...
    for(int i=1 ; i<imax ; i++)
        for(int j=1 ; j<jmax ; j++)
            ...
            {
                if(MR(pp1,0,i,j,k)!=MR(pp2,0,i,j,k)){
                    std::cout<<"Failed at: "<<"i "<<i<<" j "<<j<<" k "<<k<<std::endl;
                    successornot = false;
                    break;
                }
    ...
    if(successornot){
        std::cout<<"Success! the output files are same!"<<std::endl;
    }else{
        std::cout<<"Failed! check the code !"<<std::endl;
    }
